...
const { composeImages } = require("../../services/image.service");
...
class OutfitController {
	...
	static async composeItemImage(req, res, next) {
		try {
			const {
				imageList,
				canvasWidth = 1000,
			...
			if (
				...
				throw new ValidationError("Danh sách hình ảnh không hợp lệ");
			}
			const userId = req.user.userId;
			logger.info(`Composing item images for userId: ${userId}`);
			const filePath = await composeImages(
				imageList,
				canvasWidth,
				canvasHeight
			);
      ...
      logger.info(
        `Composed image successfully, filePath: ${filePath}`
      );
      res.sendFile(filePath)
		} catch (error) {
			logger.error(`Error composing item images: ${error.message}`);
			next(error);
		}
	}
		...
		try {
			...
			if (isNaN(pageNum) || pageNum < 1) {
				throw new ValidationError(
					"Page number phải là số nguyên dương"
				);
			}
			...
			if (filter) {
				try {
					...
					throw new BadRequestError(
						"Filter không đúng định dạng JSON"
					);
				}
			}
			...
			if (pageNum > 1) {
				...
				links.push(
					`<${baseUrl}?${queryParams.toString()}>; rel="prev"`
				);
			}
			...
			if (pageNum < totalPages) {
				queryParams.set("page", (pageNum + 1).toString());
				links.push(
					`<${baseUrl}?${queryParams.toString()}>; rel="next"`
				);
			}
	...
	static async updateOutfit(req, res, next) {
		try {
			...
			if (Object.keys(updateData).length === 0) {
				throw new BadRequestError(
					"Không có dữ liệu nào được cung cấp để cập nhật"
				);
			}

			logger.info(
				`Updating outfit, outfitId: ${outfitId}, userId: ${userId}`
			);
	...
	static async deleteOutfit(req, res, next) {
		try {
			...
			const userId = req.user.userId;
			logger.info(
				`Deleting outfit, outfitId: ${outfitId}, userId: ${userId}`
			);
	...
	static async markOutfitAsWorn(req, res, next) {
		try {
			...
			if (date && isNaN(new Date(date).getTime())) {
				throw new ValidationError(
					"Định dạng ngày không hợp lệ. Vui lòng sử dụng định dạng ISO (YYYY-MM-DD)"
				);
			}
	...
	static async addItemToOutfit(req, res, next) {
		try {
			...
			if (!position) {
				throw new ValidationError(
					"Vị trí trong trang phục (position) là bắt buộc"
				);
			}

			logger.info(
				`Adding item to outfit, outfitId: ${outfitId}, itemId: ${itemId}, position: ${position}, userId: ${userId}`
			);

			const updatedOutfit = await OutfitService.addItemToOutfit(
				outfitId,
				userId,
				itemId,
				position
	...
	static async removeItemFromOutfit(req, res, next) {
		try {
			...
			const userId = req.user.userId;
			logger.info(
				`Removing item from outfit, outfitId: ${outfitId}, itemId: ${itemId}, userId: ${userId}`
			);

			const updatedOutfit = await OutfitService.removeItemFromOutfit(
				outfitId,
				userId,
				itemId
			);
	...
	static async getOutfitRecommendations(req, res, next) {
		try {
			...
			if (isNaN(limitNum) || limitNum < 1 || limitNum > 20) {
				throw new ValidationError(
					"Limit phải là số nguyên dương và không vượt quá 20"
				);
			}

			logger.info(
				`Getting outfit recommendations, userId: ${userId}, occasion: ${occasion}, weather: ${weather}`
			);

			const recommendations =
				await OutfitService.generateOutfitRecommendations({
					userId,
					criteria: {
						occasion,
						weather,
			...
			logger.error(
				`Error getting outfit recommendations: ${error.message}`
			);
			next(error);
		}
	}
	...
	static async getOutfitStatistics(req, res, next) {
		try {
			...
			if (!validPeriods.includes(period)) {
				throw new ValidationError(
					"Period không hợp lệ. Giá trị hợp lệ: week, month, year, all"
				);
			}

			logger.info(
				`Getting outfit statistics, userId: ${userId}, period: ${period}`
			);

			const statistics = await OutfitService.getOutfitStatistics(
				userId,
				period
			);
	...
	static async getOutfitsForWeather(req, res, next) {
		try {
			...
			if (!temperature && !weatherCondition) {
				throw new ValidationError(
					"Phải cung cấp ít nhất một trong các tiêu chí: temperature, weatherCondition"
				);
			}
