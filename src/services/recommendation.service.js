...
class RecommendationService {
  ...
  static async generateAISuggestions({ userId, stylePreferences, occasion, weather }) {
    try {
      ...
      // Get existing items from user's wardrobe
      ...
      const aiSuggestion = await openAIService.generateOutfitSuggestions({
        stylePreferences: stylePreferences || userProfile.stylePreferences,
        occasion,
        weather,
        existingItems: userItems.map(item => ({
      ...
      // Create recommendation with AI suggestions
      ...
      throw new BadRequestError(`Failed to generate AI suggestions: ${error.message}`);
    }
  }
