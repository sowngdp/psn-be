...
class ItemService {
	...
	static async updateItem(itemId, userId, updateData) {
		...
		if (item.ownerId.toString() !== userId) {
			throw new ForbiddenError(
				"Bạn không có quyền cập nhật vật phẩm này"
			);
		}
	...
	static async uploadItemImage(itemId, userId, file) {
		...
		try {
			...
			if (item.imageUrl) {
				imageUrl = await firebaseStorage.updateImage(
					item.imageUrl,
					file
				);
			} else {
			...
			throw new BadRequestError(
				"Không thể xử lý hình ảnh: " + error.message
			);
		}
	}
	...
	static async getWardrobeAnalytics(userId) {
		try {
			...
			const categoryAnalytics = await itemRepository.getItemsByCategory(
				userId
			);

			// Phân tích theo mùa
			const seasonAnalytics = await itemRepository.getItemsBySeason(
				userId
			);

			// Tính toán các item được sử dụng nhiều nhất
			const mostUsedItems = await itemRepository.getMostUsedItems(
				userId,
				10
			);

			// Tính toán các item ít được sử dụng nhất
			const leastUsedItems = await itemRepository.getLeastUsedItems(
				userId,
				10
			);
	...
	static async bulkCreateItems(userId, itemsData) {
		try {
			...
			throw new BadRequestError(
				"Không thể tạo nhiều items: " + error.message
			);
		}
	}
	...
	static async importItemsFromExternal(userId, source, data) {
		try {
			...
			throw new BadRequestError(
				"Không thể import items: " + error.message
			);
		}
	}
	...
	static async genEmbeddingForAllItem() {
		...
		if (!items || items.length === 0) {
			throw new NotFoundError("Không có items nào cần tạo embedding");
		}
		const batchSize = 10;
		...
		for (let i = 0; i < totalItems; i += batchSize) {
			...
			try {
				...
				for (let j = 0; j < batch.length; j++) {
					...
					console.log(
						`Đã tạo embedding cho item ${item._id} (${
							i + j + 1
						}/${totalItems})`
					);
				}
